...
addEntry:function(a){this.$container.css("margin-top",0);var c=this.$container.height();if(!this.hasEntries){var d=b('<table class="data fullwidth"/>').prependTo(this.$container);this.$tbody=b("<tbody/>").appendTo(d)}this.$tbody.prepend('<tr><td><a href="'+a.url+'">'+a.title+'</a> <span class="light">'+(a.dateCreated?Craft.formatDate(a.dateCreated):"")+(a.dateCreated&&a.username&&Craft.edition>=Craft.Client?", ":"")+(a.username&&Craft.edition>=Craft.Client?a.username:"")+"</span></td></tr>");a=this.$container.height()-
c;this.$container.css("margin-top",-a);a={"margin-top":0};this.hasEntries||(a["margin-bottom"]=-c,this.hasEntries=!0);this.$container.velocity(a)},destroy:function(){Craft.RecentEntriesWidget.instances.splice(b.inArray(this,Craft.RecentEntriesWidget.instances),1);this.base()}},{instances:[]})})(jQuery);
